    ...
    <main className="mx-auto min-h-screen w-full max-w-7xl px-4 py-6 sm:px-6 lg:py-10">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Ton radar personnel · signaux techniques · zones de repli</p>
        </div>
      ...
      {isLoading && !data ? <SkeletonState /> : data && (
        <>
          ...
          <section className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
            <div>
              <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-muted-foreground">⭐ Actions favorites</h2>
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">{data.stocks.map((q) => <StockCard key={q.symbol} q={q} />)}</div>
              <h2 className="mb-3 mt-10 text-xs font-semibold uppercase tracking-widest text-muted-foreground">📊 FNB favoris</h2>
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">{data.etfs.map((q) => <StockCard key={q.symbol} q={q} />)}</div>
            </div>
            <div className="lg:sticky lg:top-6 lg:self-start"><Opportunities quotes={[...data.stocks, ...data.etfs]} /></div>
          </section>

          <section className="mt-8 rounded-lg border bg-card p-4">
            <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Comment lire les signaux personnalisés</h3>
            <div className="mt-3 grid grid-cols-1 gap-x-8 gap-y-2 text-xs text-muted-foreground sm:grid-cols-2 lg:grid-cols-3">
              <p><span className="font-semibold text-bull">🟢 Acheter maintenant</span> — score élevé et configuration technique encore raisonnable.</p>
              <p><span className="font-semibold text-primary">🔵 Acheter sur repli</span> — bon dossier technique, mais zone support/MM50 plus intéressante pour l'entrée.</p>
              <p><span className="font-semibold text-neutral">🟡 Surveiller</span> — tendance constructive mais confirmation nécessaire.</p>
              <p><span className="font-semibold text-warn">🟠 Attendre</span> — signaux mitigés ou prix trop éloigné de la zone technique.</p>
              <p><span className="font-semibold text-bear">🔴 Éviter</span> — tendance/momentum défavorables.</p>
              <p className="text-muted-foreground/80 sm:col-span-2 lg:col-span-1">La zone d'achat est une estimation technique basée sur support récent + MM50. Elle n'est pas une cible de cours ni un conseil financier.</p>
            </div>
          </section>
