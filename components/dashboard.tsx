...
export function Dashboard() {
  ...
  const radarQuotes = useMemo(() => data ? sortQuotes([...data.stocks, ...data.etfs], sort).slice(0, 5) : [], [data, sort])
  ...
  return <main className="mx-auto min-h-screen w-full max-w-7xl px-4 py-5 sm:px-6 lg:py-8">
    ...
    {isLoading && !data ? <SkeletonState /> : data && <>
      {tab === 'radar' && <section className="mt-5 rounded-2xl border border-primary/20 bg-card p-4 shadow-sm sm:p-5"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2"><Flame className="h-5 w-5 text-primary" /><h2 className="text-lg font-bold text-card-foreground">🔥 Radar d'achat — {sort === 'potential' ? 'Potentiel 12 mois' : sort === 'score' ? 'Score technique' : sort === 'momentum' ? 'Momentum' : sort === 'rsi' ? 'RSI bas' : sort === 'drawdown' ? 'Drawdown' : 'Alphabétique'}</h2></div><p className="mt-1 text-xs text-muted-foreground">Le classement ci-dessous change immédiatement selon le critère sélectionné.</p></div><select value={sort} onChange={(e) => setSort(e.target.value as Sort)} className="w-fit rounded-md border bg-background px-2.5 py-1.5 text-xs"><option value="potential">Potentiel 12 mois</option><option value="score">Technique</option><option value="momentum">Momentum</option><option value="rsi">RSI bas</option><option value="drawdown">Drawdown</option><option value="name">Alphabétique</option></select></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">{radarQuotes.map((q) => <StockCard key={q.symbol} q={q} compact />)}</div><div className="mt-4 border-t border-border pt-4"><Opportunities quotes={[...data.stocks, ...data.etfs]} /></div></section>}
