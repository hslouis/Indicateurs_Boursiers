...
function potential(q: Quote) {
  ...
  return Math.max(0, Math.min(100, Math.round(a.score * 0.65 + Math.min(100, Math.max(0, momentum * 3 + 50)) * 0.2 + Math.min(100, room * 5) * 0.15)))
}

function sortQuotes(quotes: Quote[], sort: Sort) {
  return [...quotes].sort((a, b) => {
    if (sort === 'potential') return potential(b) - potential(a)
...
export function Dashboard() {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-7xl px-4 py-5 sm:px-6 lg:py-8">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2"><Activity className="h-5 w-5 text-primary" /><h1 className="text-balance text-2xl font-bold tracking-tight sm:text-3xl">Indicateurs Boursiers</h1></div><p className="mt-1 text-sm text-muted-foreground">Ton radar personnel · potentiel technique · timing d'achat</p></div><div className="flex items-center gap-3"><div className="flex items-center gap-2 rounded-md border bg-card px-3 py-1.5"><span className={`h-2 w-2 rounded-full ${isOpen ? 'bg-bull animate-pulse-dot' : 'bg-muted-foreground'}`} /><span className="text-xs font-medium text-card-foreground">{marketState ? marketStateLabel(marketState) : 'Chargement…'}</span></div><button onClick={handleRefresh} disabled={manualLoading} className="flex items-center gap-2 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-60"><RefreshCw className={`h-3.5 w-3.5 ${manualLoading ? 'animate-spin' : ''}`} /> Actualiser</button></div></header>
    ...
    {isLoading && !data ? <SkeletonState /> : data && <>
      ...
      <section className="mt-5"><h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-muted-foreground">Marchés de référence</h2><IndexStrip indices={data.indices} /></section>
      {tab === 'radar' ? <section className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_380px]"><div><div className="mb-3 flex items-center justify-between"><div><h2 className="text-lg font-bold text-card-foreground">🔥 Top occasions</h2><p className="text-xs text-muted-foreground">Les titres avec le meilleur potentiel technique actuel.</p></div><select value={sort} onChange={(e) => setSort(e.target.value as Sort)} className="rounded-md border bg-card px-2 py-1.5 text-xs"><option value="potential">Potentiel</option><option value="score">Signal</option><option value="momentum">Momentum</option><option value="rsi">RSI bas</option><option value="drawdown">Drawdown</option></select></div><div className="grid grid-cols-1 gap-3 xl:grid-cols-2">{sortQuotes([...data.stocks, ...data.etfs], 'potential').slice(0, 6).map((q) => <StockCard key={q.symbol} q={q} compact />)}</div></div><div className="lg:sticky lg:top-5 lg:self-start"><Opportunities quotes={[...data.stocks, ...data.etfs]} /></div></section> : <section className="mt-6"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-bold text-card-foreground">{tab === 'stocks' ? '⭐ Actions favorites' : tab === 'etfs' ? '📊 FNB favoris' : '📈 Indices'}</h2><p className="text-xs text-muted-foreground">Classés selon le critère sélectionné.</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setPortfolioOnly(!portfolioOnly)} className={`rounded-md border px-3 py-1.5 text-xs font-semibold ${portfolioOnly ? 'border-primary bg-primary/10 text-primary' : 'bg-card text-muted-foreground'}`}>{portfolioOnly ? '✓ En portefeuille' : 'En portefeuille'}</button><select value={sort} onChange={(e) => setSort(e.target.value as Sort)} className="rounded-md border bg-card px-3 py-1.5 text-xs"><option value="potential">Potentiel de gain</option><option value="score">Score technique</option><option value="momentum">Momentum</option><option value="rsi">RSI bas</option><option value="drawdown">Drawdown</option><option value="name">Alphabetique</option></select></div></div><div className="grid grid-cols-1 gap-3 xl:grid-cols-2">{visible.map((q) => <StockCard key={q.symbol} q={q} compact />)}</div></section>}
...
function SkeletonState() { return <div className="mt-6 space-y-6"><div className="h-12 animate-pulse rounded-lg border bg-card" /><div className="grid grid-cols-1 gap-3 sm:grid-cols-3">{[0,1,2].map((i) => <div key={i} className="h-24 animate-pulse rounded-lg border bg-card" />)}</div><div className="grid grid-cols-1 gap-3 lg:grid-cols-2">{[0,1,2,3].map((i) => <div key={i} className="h-52 animate-pulse rounded-xl border bg-card" />)}</div></div> }
